...
//    https://api.weixin.qq.com/sns/auth?access_token=ACCESS_TOKEN&openid=OPENID
    ...
    if ($uid) {
        ...
        $u = M('user');
        $user = $u->where(array('openid' => $uid))->find();
        $token = $user['token'];
        $openid = $user['openid'];
        ...
        $check_auth_url = 'https://api.weixin.qq.com/sns/auth?access_token=' . $token . '&openid=' . $openid;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $check_auth_url);
